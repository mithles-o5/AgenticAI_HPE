...
class ABACEngine:
    ...
    """
    CRITICAL_ACTIONS = ["delete", "execute", "update"]
    ...
    def evaluate(self, payload: TokenPayload, action: str, resource: Resource, context: Context) -> tuple[bool, str]:
        ...
        if resource.vendor.upper() == "HPE":
            # Example rule: HPE vendor resources require specific handling or higher privileges for executes
            if action in ["execute", "delete"] and role == "operator":
                return False, "Operators cannot execute or delete HPE vendor resources."
